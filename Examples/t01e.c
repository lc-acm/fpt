...
int main()
{
   ...
   G_draw_string (word,   300,80) ;

   // you'll need to compile with bcom, because of your
   // use of a the double, i, as a subscript
   unpack(word, word) ;
   i = 0 ;
   while (word[i] != 0) {
     word[i] += 2.0 ;
   ...
   G_rgb(1,0,1) ;
   //   G_rectangle(0,0,swidth,sheight) ;
   G_line(0,0, 0,sheight-1) ;
   G_line(0,sheight-1, swidth-1, sheight-1) ;
   G_line(swidth-1, sheight-1, swidth-1, 0) ;
   G_line(swidth-1, 0,  0,0) ;

   q = G_wait_key() ;    // pause again before exit 

   G_save_image_to_file ("t01e.xwd") ;
